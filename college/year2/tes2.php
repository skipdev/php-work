...
<html>
	...
	<body>
		...
		if (  isset($_GET["one"]) && isset($_GET["two"]) && ($_GET["three"]="&")) {
			echo "box one * box two = " . $_GET["one"] * $_GET["two"];
		}
		
		echo "<br>";
		
		if ( isset($_GET["one"]) && isset($_GET["three"]) && ($_GET["two"]="&")){
			echo "box one * box three = " . $_GET["one"] * $_GET["three"];
		}
		
		echo "<br>";
		
		if ( isset($_GET["two"]) && isset($_GET["three"]) && ($_GET["one"]="&")){
			echo "box two * box three = " . $_GET["two"] * $_GET["three"];
		}
		...
		if ( isset($_GET["two"]) && isset($_GET["three"]) && isset($_GET["one"]) ){
			echo "box one + two + three = " . $_GET["two"] + $_GET["one"] + $_GET["three"];
		}
